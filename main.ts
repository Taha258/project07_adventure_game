...
//slow animationtext
async function slowtext(text: string, delay = 20) {
  ...
  async gameObj() {
    ...
    while (running) {
      ...
      while (opponentHealth > 0) {
        ...
        switch (answer.Act) {
          case "Attack":
            let damageDealt: number = Math.floor(random * attackDamage);
            let damageTaken: number = Math.floor(random * maximumHealthDamage);
